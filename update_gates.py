...
start_sig = r"void drawGate\(bool lit, bool isExit, bool isLeftWall\) \{"
end_sig = r"\}\n\nvoid drawWallSpeakers\(\) \{"

match = re.search(start_sig + r".*?(?=\}\n\nvoid drawWallSpeakers\(\) \{)", text, re.DOTALL)
if match:
    new_gate_code = """void drawGate(bool lit, bool isExit, bool isLeftWall) {
    float wallX = isLeftWall ? -16.8f : 16.8f; 
    float doorZ = 10.0f; // Align both precisely in front of each other (middle of seating)
    ...
    glPopMatrix();"""
    text = text[:match.start()] + new_gate_code + text[match.end():]
